...
class CatalogTest extends TestCase
{
    ...
    public function testCHeckGate()
    {
        ...
        \Gate::define('testExchangeGateFalse', function ($user) {
            return false;
        });

        \Gate::define('testExchangeGateTrue', function ($user) {
            return true;
        });
    ...
    protected function createTmpFileToSend()
    {
        ...
        \File::put($path,
            \App::make(\Faker\Generator::class)->realText(5000));

        if (function_exists("zip_open") && class_exists(Zipper::class)) {
            $zipName = __DIR__.'/'.$fileName.'.zip';

            \File::delete($zipName);

            \Zipper::make($zipName)->add($path)->close();
